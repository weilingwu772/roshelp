...
規範工業/軍事等級的安全需求。
- **Authentication (身分驗證)**：確認誰有資格加入通訊網。
- **Access Control (存取控制)**：誰可以 Publish 哪一個 Topic。
- **Cryptography (加密)**：資料在線路上的加密規則。
...
3.  **Actions (動作)**
    ...
    - **用途**：導航、機械臂執行路徑。
4.  **Parameters (參數)**
    - **用途**：節點設定值，如最大速度、PID 參數等。
    - **標準化分類 (根據 REP 建議)**：
        - **Identity & Connection**：身分與連線設定（如 `frame_id`、`device_id`、連線埠）。
        - **Transformation - TF**：座標轉換參數（如感測器安裝的偏移量 `offset_x/y/z`）。
        - **Performance**：效能調優（如 `update_rate` 更新頻率、`timeout` 逾時設定）。
        - **Data Quality**：資料過濾與精確度（如 `min_range` 範圍限制、`threshold` 門檻值）。

### 3.3 協定與介面 (Interface & Protocol)

如果「通訊機制」是物流模式，「協定與介面」就是**包裹規格**與**運輸標準**。

- **介面格式 (Interfaces)**
    - **`.msg` (Message)**：定義 Topic 的內容（如：雷達數據包含角度、距離）。
    - **`.srv` (Service)**：定義「請求」與「回應」的雙向結構。
    - **`.action` (Action)**：定義「目標、進度反饋、最終結果」的三段式格式。
    - 實現**語言無關性**。不管是 C++ 或 Python 寫的節點，只要引用同一份 `.msg` 定義，就能百分之百確認資料結構一致。

- **硬體解耦 (Hardware Decoupling)**
    - **核心優勢**：ROS 將底層物理協定（如 Serial, CAN, USB）抽象化。開發者只需要關注 ROS 標準介面，不需要在演算法層次直接面對瑣碎的硬體通訊細節。

- **傳輸協議：RTPS (Real-Time Publish-Subscribe)**
    - **核心優勢**：這是 ROS 2 實踐**去中心化**的關鍵，也是 DDS 的標準連線協議：
    - **自動發現 (Discovery)**：節點啟動後會「自報家門」，自動尋找網域內匹配的發佈者或訂閱者，不再依賴 `roscore` 中心伺服器。

- **連線規範 (Wire Protocol)**：定義資料在網路上流動的二進位格式，確保不同廠商的 DDS 實作可以互通。

#### 4. 常見硬體傳輸介面 (Common Hardware Interfaces)
ROS 透過驅動節點 (Driver Node) 將以下物理協定轉化為標準 Topic：

*   **TCP/IP (網路通訊)**：頻寬大、距離遠。用於雷達 (LiDAR)、深度相機或分散式多機通訊。
*   **Serial Port (RS232 / RS485)**：工業級穩定通訊。常用於控制馬達驅動器或遠距離感測器。
*   **UART (晶片層級通訊)**：最基礎、低成本。用於微控制器 (MCU) 與主控電腦間的資料交換。
*   **GMSL2 (車規高速連結)**：超高頻寬、極低延遲。專為自動駕駛相機 (如 GMSL 相機) 設計。

---
#### 在ROS世界內互連裝置，因為大家已經講一樣的語言，所以沒有傳遞需轉換的問題，除非是需要跟非ROS世界的東西溝通，像是將相機圖資即時連到遠端監控的網頁，就需處理以下三個：
1. A和B兩端文字統一，JSON資料格式，讓要傳遞的靜態文字兩邊都讀的懂（例如 {"speed": 1.5, "status": "OK"}）
2. 設定A到B的傳遞細節，也就是API，包含傳遞路徑(通常遵循 RESTful 原則)、傳遞後做什麼動作(例如GET、POST、DELETE)、回應狀態碼(例如200 OK表達完成傳遞)
3. 產生A到B的傳遞行為，這個行為就稱Bridge(橋接器)，通常是一個執行轉換的程式，例如ROS內部資料是Topic，網頁看不懂，Bridge就會攔截這些Topic，照著API規定的細節，再將它包裝成網頁看得懂的JSON格式傳過去。

---

### 3.4 零組件廠商的 ROS 支援標配 (Component Support)
為了確保硬體能快速整合進 ROS 系統，專業供應商應提供：

*   **SDK 套件**：
    *   標準 ROS Node (支援多執行緒/組件化)。
    *   清楚的 Topic/Service/Action 定義文件。
*   **模擬支援**：
    *   提供 **Xacro/URDF 模型**（含碰撞模型與慣性矩物理屬性）。
    *   提供專屬的 **RViz 配置檔 (.rviz)**，讓開發者一眼看出數據是否正常。
*   **部署工具**：
    *   範例 **Launch 檔案** (需支援 Namespace、Remapping 與參數覆蓋)。
    *   提供 **Dockerfile**，解決環境相依性問題。
*   **維護與文件**：
    *   GitHub 上的開源測試案例與 Library。
    *   C++/Python 雙語言範例代碼。
