...
### 3. 傳輸規則 QoS (Quality of Service)
預設 20 多種傳輸規則，開發者可自行設定或切換。
- **Reliability**：可靠性（Reliable vs Best Effort）。
- **Durability**：持久性（資料發布後，晚進來的訂閱者是否還收得到歷史資料）。
- **Liveliness**：活性檢測（確認對方節點還活著）。
...
---
### 💡 總結
簡單來說，DDS 規範了：**「程式怎麼寫 (API)」、「網路怎麼傳 (Protocol)」、「傳輸規則 (QoS)」、「資料長什麼樣 (XTypes/IDL)」** 以及 **「安全性 (Security)」**。這也是為什麼 ROS 2 選擇 DDS 作為底層後，可以獲得商用等級的穩定性。



- **以資料為中心 (Data-Centric)**：通訊不依賴特定的節點身分，而是關注資料本身在「全局空間」的流動與過濾。
- **去中心化與自動發現**：節點之間透過 **Discovery** 機制自動發現彼此，不再依賴單一的 Master (roscore) 管理通訊。
- **內建預設設定檔**：可以直接選擇Profile，如 `Sensor Data` 模式，不需要手動微調所有參數。
- **Reliability (可靠性)**：
    - `Reliable`: 確保資料必達（像傳簡訊，適合指令或重要狀態）。
    - `Best Effort`: 只求快，丟包不重傳（像看直播，適合感測器大數據）。
- **Durability (持久性)**：新加入的訂閱者是否能拿到之前的舊資料（例如地圖資料）。
...
### 2.3 協定與介面 (Interface & Protocol) —— 溝通的「語言」與「規則」
...
## 8. 結論：從 Prototype 到 Product 的橋樑
